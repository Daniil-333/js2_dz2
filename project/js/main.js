class Products {
    data = [];
    products = [];
    container = null;

    constructor(selector) {
        this.container = document.querySelector(selector);
        this._fetchData();
        this._render();
    }

    totalSumPrice() {
        this.data.reduce(function (accumPrice, currentValue) {
            return accumPrice+currentValue.price;
        }, 0)
    }

    _fetchData() {
        this.data = [
            { title: 'Notebook', id: 1, price: 2000 },
            { title: 'Keyboard', id: 2, price: 200 },
            { title: 'Mouse', id: 3, price: 100 },
            { title: 'Gamepad', id: 4, price: 87 }
        ];
    }

    _render() {
        for (let data of this.data) {
            const product = new ProductItem(data);
            this.products.push(product);
            this.container.insertAdjacentHTML('beforeend', product.render())
        }
    }
}

class ProductItem {
    title = '';
    price = 0;
    id = 0;
    img = '';

    constructor(product, img = 'https://placehold.it/200x150') {
        ({ title: this.title, price: this.price, id: this.id } = product);
        this.img = img;
    }

    render() {
        return `<div class="product-item">
                 <img src="${this.img}" alt="${this.title}">
                 <div class="desc">
                     <h3>${this.title}</h3>
                     <p>${this.price}</p>
                     <button class="buy-btn">Купить</button>
                 </div>
             </div>`
    }
}


class Cart {
    // items - массив всех товаров, находящихся в корзине
    // itemsContainer - элемент, где будут находиться все товары корзины

    // render() - отображение разметки всех товаров корзины
    // removeItem() - удаление одного товара из корзины
    // clear() - очистка корзины
    // makePercentDiscount(arg) - применение скидки, в качестве аргумента раззмер скидки в процентах
    // subtotal() - промежуточная стоимость всех товаров корзины(до применения скидки, если таковая имеется)
    // grandTotal() - итоговая стоимость всех товаров корзины
    // getQuote() - Я не совсем понял, о чём здесь речь? Но ощущение есть, что должен быть реализован какой-то метод. На вид это обычная форма для заполнения адреса доставки. Если натолкнёте меня на мысль, буду очень признателен)
}

class CartItem {
    // name - название товара
    // color - цвет товара
    // size - размер товара
    // img - картинка товара
    // unitPrice - цена за единицу товара
    // quantity - количевто товара
    // shipping - доставка (если бесплатная, то String "FREE", если платная, то Number)

    // subtotal() - промежуточная стоимость товара
    // render() - генерация разметки одного товара корзины
}

const list = new Products('.products');




